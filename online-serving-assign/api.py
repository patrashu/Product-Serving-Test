...
def get_predictions() -> list[PredictionResponse]:
    with Session(engine) as session:
        statement = select(PredictionResult)  # 선택해서 받아오고
        prediction_results = session.exec(statement).all()  ## 모든 데이터를 받아옴
...
def get_prediction(id: int) -> PredictionResponse:
    with Session(engine) as session:
        ...
        if not prediction_result:
            raise HTTPException(
                detail="Prediction not found", status_code=status.HTTP_404_NOT_FOUND
            )
        return PredictionResponse(id=prediction_result.id, result=prediction_result.result)